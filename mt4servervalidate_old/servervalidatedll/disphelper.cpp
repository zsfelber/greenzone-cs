...
char c0 = 0;
VARIANT emptyStrVariant = {VT_BSTR,0,0,0};
			    ...
			    COSERVERINFO * pServerInfo, void ** ppv)
{
	...
	CLSID *pclsid = &clsid;//(REFIID)
	IID *piiclf = &(IID)IID_IClassFactory;
	if (SUCCEEDED(hr)) hr = CoGetClassObject((REFCLSID)pclsid, dwClsContext, pServerInfo, (REFIID)piiclf, (void **) &pCf);
		         ...
		         DWORD dwClsContext, LPVOID lpvReserved, void ** ppv)
{
	...
	if (szPathName)
	{
		...
		else
		{
			...
			IID *iipf = &(IID)IID_IPersistFile;
			hr = dhCreateObjectEx(szProgId, (REFIID)iipf, dwClsContext, NULL, (void **) &ppf);
	...
	else
	{
		...
		if (L'{' == szProgId[0])
			...
			hr = CLSIDFromProgID(szProgId, &clsid);
		IID *pclsid = &(IID)clsid;//(REFIID)
		if (SUCCEEDED(hr)) hr = GetActiveObject((REFIID)pclsid, NULL, &pUnk);
...
HRESULT dhCreateObject(LPCOLESTR szProgId, LPCWSTR szMachine, IDispatch ** ppDisp)
{
	...
	IID *iidp = &(IID)IID_IDispatch;
	return DH_EXIT(dhCreateObjectEx(szProgId, (REFIID)iidp,
...
HRESULT dhGetObject(LPCOLESTR szPathName, LPCOLESTR szProgId, IDispatch ** ppDisp)
{
	...
	IID *iidp = &(IID)IID_IDispatch;
	return DH_EXIT(dhGetObjectEx(szPathName, szProgId, (REFIID)iidp,
                         ...
                         IDispatch * pDisp, LPCOLESTR szMember, VARIANT * pArgs)
{
	...
	IID *iinull = &(IID)IID_NULL;
	hr = pDisp/*->lpVtbl*/->GetIDsOfNames(/*pDisp, */(REFIID)iinull, (LPOLESTR *) &szMember, 1, LOCALE_USER_DEFAULT, &dispID);
	...
	iinull = &(IID)IID_NULL;
	hr = pDisp/*->lpVtbl*/->Invoke(/*pDisp, */dispID, (REFIID)iinull, LOCALE_USER_DEFAULT, (WORD) invokeType, &dp, pvResult, &excep, &uiArgErr);
                               ...
                               IDispatch * pDisp, LPOLESTR szMember, va_list * marker)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		{
			if (pvResult->vt == VT_EMPTY || pvResult->vt == VT_NULL)
			{
				if (returnType == VT_EMPTY || returnType == VT_NULL)
				{
					VariantClear(pvResult);
				}
				else
				{
					emptyStrVariant.pcVal=&c0;
					hr = VariantChangeType(pvResult, &emptyStrVariant, 16 , returnType);
				}
			}
			else if (pvResult->vt != returnType)
			{
...
HRESULT dhEnumBeginV(IEnumVARIANT ** ppEnum, IDispatch * pDisp, LPCOLESTR szMember, va_list * marker)
{
	...
	IID *iinull = &(IID)IID_NULL;
	hr = pDispObj/*->lpVtbl*/->Invoke(/*pDispObj, */DISPID_NEWENUM, (REFIID)iinull, LOCALE_USER_DEFAULT,
	...
	IID *iienum = &(IID)IID_IEnumVARIANT;
	if (vtResult.vt == VT_DISPATCH)
		hr = vtResult.pdispVal/*->lpVtbl*/->QueryInterface(/*vtResult.pdispVal, */(REFIID)iienum, (void **) ppEnum);
	else if (vtResult.vt == VT_UNKNOWN)
		hr = vtResult.punkVal/*->lpVtbl*/->QueryInterface(/*vtResult.punkVal, */(REFIID)iienum, (void **) ppEnum);
	else
		hr = E_NOINTERFACE;
                 ...
                 EXCEPINFO * pExcepInfo, UINT iArgError, LPCWSTR szFunctionName)
{
	...
	{
		PDH_EXCEPTION pException = (PDH_EXCEPTION)GetExceptionPtr();

		if (!pException)
		{
			pException = (PDH_EXCEPTION)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(DH_EXCEPTION));
...
HRESULT dhGetLastException(PDH_EXCEPTION * ppException)
{
	...
	CheckTlsInitialized();
	*ppException = (PDH_EXCEPTION)GetExceptionPtr();
...
void dhCleanupThreadException(void)
{
	...
	CheckTlsInitialized();
	pException = (PDH_EXCEPTION)GetExceptionPtr();
