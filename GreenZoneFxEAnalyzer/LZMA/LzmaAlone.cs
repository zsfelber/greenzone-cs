...
namespace SevenZip
{
	...
	public class CDoubleStream: Stream
	{
		...
		{
			throw (new LZMAException("can't Write"));
		}
		public override long Seek(long offset, System.IO.SeekOrigin origin)
		{
			throw (new LZMAException("can't Seek"));
		}
		public override void SetLength(long value)
		{
			throw (new LZMAException("can't SetLength"));
		}
	}
	...
	class LzmaAlone
	{
		...
		{
			throw (new LZMAException("Command line error"));
			// System.Console.WriteLine("\nCommand line error\n");
			// return 1;
		}
		static int Main2(string[] args)
		{
			...
			if (stdInMode)
			{
				throw (new LZMAException("Not implemeted"));
			}
			else
			{
				if (paramIndex >= nonSwitchStrings.Count)
			...
			else if (command == "d")
			{
				...
				if (trainStream != null)
				{
					if (!decoder.Train(trainStream))
						throw (new LZMAException("can't train"));
				}
				long outSize = 0;
				for (int i = 0; i < 8; i++)
				{
				...
				throw (new LZMAException("Command Error"));
			return 0;
		}
