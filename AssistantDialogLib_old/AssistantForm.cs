...
namespace SMS.Windows.Forms
{
    ...
    public class AssistantForm : Form
	{
        ...
        public AssistantPage[] Pages
        {
            get
            {
                return (AssistantPage[])m_pages.ToArray(typeof(AssistantPage));
            }
        }
        ...
        private void ActivatePage( int newIndex )
        {
            ...
            AssistantPage currentPage = null;
            if( m_selectedIndex != -1 )
            {
                currentPage = (AssistantPage)m_pages[ m_selectedIndex ];
                if( !currentPage.OnKillActive() )
                    return;
            }

            // Activate the new page
            AssistantPage newPage = (AssistantPage)m_pages[ newIndex ];
        ...
        private void OnClickBack( object sender, EventArgs e )
        {
            ...
            if( m_selectedIndex != -1 )
            {
                AssistantPage cur = (AssistantPage)m_pages[
                    m_selectedIndex];
                ...
                switch( pageName )
                {
                    ...
                    case NextPage:
                        if (m_selectedIndex - 1 >= 0)
                        {
                            ...
                            ActivatePage(m_pages.IndexOf(cur.PreviousPage));
                        }
                        break;

                    // Activate the specified page if it exists
                    default:
                        foreach( AssistantPage page in m_pages )
                        {
        ...
        private void OnClickFinish( object sender, EventArgs e )
        {
            ...
            if( m_selectedIndex != -1 )
            {
                ...
                AssistantPage page = (AssistantPage)m_pages[ m_selectedIndex ];
        ...
        protected override void OnControlAdded( ControlEventArgs e )
        {
            ...
            // this form
            AssistantPage page = e.Control as AssistantPage;
            if( page != null )
            {
                page.Visible = false;
        ...
        /// <c>AssistantPage.OnSetActive</c>.  You can display a Finish or a
