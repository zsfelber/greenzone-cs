...
public class Parser
{
	...
	private string Lookahead(int CharIndex)
	{
		...
		if (CharIndex <= m_LookaheadBuffer.Length) {
			return ""+m_LookaheadBuffer[CharIndex - 1];
		} else {
			return "";
		}
	}
			...
			while (!(EGT.EndOfFile() | Success == false)) {
				...
				switch (RecType) {
					...
					case EGTRecord.Group:
						...
						_with1.Container = m_SymbolTable[EGT.RetrieveInt16()];
                        _with1.Start = m_SymbolTable[EGT.RetrieveInt16()];
                        _with1.End = m_SymbolTable[EGT.RetrieveInt16()];
					...
					case EGTRecord.CharRanges:
						...
						while (!(EGT.RecordComplete())) {
                            m_CharSetTable[Index].Add(new CharacterRange((ushort)EGT.RetrieveInt16(), (ushort)EGT.RetrieveInt16()));
						}
	...
	private Token ProduceToken()
	{
		...
		while (!Done) {
			...
			if (NestGroup) {
				ConsumeBuffer(Read.SData.Length);
				...
				if (m_GroupStack.Count == 0) {
					...
					m_GroupStack.Top().SData += Pop.Data;
				...
				if (Top.Group().Advance == Group.AdvanceMode.Token) {
					...
					//Top.Data += Read.SData.Chars(0);
                    Top.SData += Read.SData[0];
